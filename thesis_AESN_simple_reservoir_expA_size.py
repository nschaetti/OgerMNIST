	...
	for size in rc_Size:
		for spectral_radius in rc_SpectralRadius:
			for leak_rate in rc_LeakRate:
				#print "Paramètre rc_SpectralRadius={},rc_LeakRate={},rc_Size={}".format(spectral_radius, leak_rate, size)
				for n in np.arange(0,rc_SampleSize):
					...
					reservoir = Oger.nodes.LeakyReservoirNode(leak_rate = leak_rate, input_dim = digitImport.nbInputs, output_dim = size, input_scaling = rc_InputScaling, spectral_radius = spectral_radius, bias_scaling=rc_Biais, sparsity = rc_InputSparsity, w_sparsity = rc_ReservoirSparsity)
				...
				print '\033[93m' + "Paramètre rc_SpectralRadius={},rc_LeakRate={},rc_Size={} : moyenne de {}".format(spectral_radius, leak_rate, size, np.average(results)) + '\033[0m'

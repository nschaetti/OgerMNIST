...
rc_SpectralRadius 			= np.arange(1.6,3.1,0.1)				# Spectral radius
rc_Size 					= 100				# Taille du réservoir
rc_InputScaling 			= 0.5				# Dimensionnement des entrées
rc_LeakRate					= np.arange(0.1,1.1,0.1)				# Leak rate
rc_Bias						= 0.4
...
if __name__ == "__main__":
	...
	for spectral_radius in rc_SpectralRadius:
		for leak_rate in rc_LeakRate:
			...
			for n in np.arange(rc_SampleSize):
				# Réservoir et jointure
				reservoir = Oger.nodes.LeakyReservoirNode(input_dim = digitImport.nbInputs, output_dim = rc_Size, input_scaling = rc_InputScaling, spectral_radius = spectral_radius, leak_rate = leak_rate, bias_scaling = rc_Bias)
				...
				results[n] = der
			print '\033[93m' + "rc_SpectralRadius={},rc_LeakRate={} : moyenne de {}".format(spectral_radius,leak_rate,np.average(der)) + '\033[0m'

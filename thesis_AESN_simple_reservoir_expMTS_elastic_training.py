...
rc_TrainingLength			= np.arange(61000,120001,5000)				# Longueur d'entrainement
...
if __name__ == "__main__":
	...
	for training_length in rc_TrainingLength:
		...
		train_results = np.zeros(rc_SampleSize) 
		for n in np.arange(0,rc_SampleSize):
			...
			train_der, _, _, _ = digitImport.digitErrorRateTrainging(trainout, with_miss_array = True, per_digit = True, pos_table = True)
			results[n] = der
			train_results[n] = train_der
		
		print '\033[93m' + "Paramètre rc_SpectralRadius={},rc_LeakRate={},rc_TrainingLength={} : moyenne de {}/{}".format(rc_SpectralRadius, rc_LeakRate, training_length, np.average(results), np.average(train_results)) + '\033[0m'

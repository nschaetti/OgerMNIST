...
digtlen = load('./inputs/load_lengthfile1.dat');
...
load(['./inputs/demo3/mask_' num2str(Nnode) '.mat']);
...
if (nWi==0)
    ...
    for m=1:Nsd
        % load a cochleagram
        Mc=load(['./inputs/load_inputs1_' num2str(m) '.dat']);
...
%Laurent fait une boucle sur ce paramètre pour l'optimiser, on va changer
%cela par la suite :-)
for C=23%1:Cmax
    %
    % Temporal parameters
   ...
   % Dgv=zeros(1,Nsd);
    ...
    for p=1:Nttmax
        ...
        for m=1:Ndptt
            % load a cochleagram
            Mc=load(['./inputs/load_inputs1_' num2str(prm(Ndptt*(p-1)+m)) '.dat']);
            % construct the input with the mask
            Mu=Wi*Mc;
            %

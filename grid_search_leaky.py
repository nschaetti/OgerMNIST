			...
			for input_scaling in rc_InputScaling:
				for leak_rate in rc_LeakRate:
					for bias in rc_Bias:
						...
						cur.execute("SELECT * FROM analyze_sr_is_size_lr WHERE size = {} AND spectral_radius = {} AND input_scaling = {} AND leak_rate = {} AND bias = {}".format(size,spectral_radius,input_scaling,leak_rate,bias))
						rows = cur.fetchall()
						...
						if len(rows) == 0:
							...
							insert = "INSERT INTO analyze_sr_is_size_lr (`id`, `size`, `spectral_radius`, `input_scaling`, `leak_rate`, `bias`, `digit_error_rate`) VALUES (NULL, '{}', '{}', '{}', '{}', '{}', '{}');".format(size,spectral_radius,input_scaling,leak_rate,bias,count/3.0)
							print "Insertion... {}".format(insert)
							cur.execute(insert)
							con.commit()
						else:
							print "OK : INSERT INTO analyze_sr_is_size_lr (`id`, `size`, `spectral_radius`, `input_scaling`, `leak_rate`, `bias`, `digit_error_rate`) VALUES (NULL, '{}', '{}', '{}', '{}', '{}');".format(size,spectral_radius,input_scaling,leak_rate,bias)

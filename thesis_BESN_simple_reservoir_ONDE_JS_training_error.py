...
if __name__ == "__main__":
	...
	for training_length in rc_TrainingLength:
		...
		for n in np.arange(0,rc_SampleSize):
			...
			joiner = JoinedStatesNode(image_size = rc_NbSteps, input_dim = rc_Size)
			...
			results[n] = der
			# Digit error rate
			train_der, misses, per_digit, miss_pos = digitImport.digitErrorRateTrainging(trainout, with_miss_array = True, per_digit = True, pos_table = True)
			train_results[n] = train_der
		print '\033[93m' + "rc_SpectralRadius={},rc_LeakRate={},rc_TrainingLength={} ::: {}/{}".format(rc_SpectralRadius,rc_LeakRate,training_length,np.average(results),np.average(train_results)) + '\033[0m'
